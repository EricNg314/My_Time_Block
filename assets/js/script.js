...
function renderRow(time, task){
  ...
  if(timeRowSearch.length === 0){
    ...
    labelEle = $(`<label class="col-2 col-lg-1 p-3 hour " >${localTime + ":" + timeArr[1] + " " + amPm}</label>`);
    textareaEle = $(`<textarea class="col-8 col-lg-10 form-control  ${timeBgClassStr}" placeholder="" >${task}</textarea>`);
...
function getPastPresentFuture(timeArr){
  ...
  var rowDateTime = luxon.DateTime.fromObject({hour: timeArr[0], minute: timeArr[1] }).toObject();
